...
public class MOEADD extends AbstractMOEAD<DoubleSolution> {
    ...
    int numRanks;
    HashMap<Solution, Integer> solutionsRanking;
    HashMap<Solution, Integer> solutionsRegion;
    HashMap<Solution, Double> solutionsDistance;
    ...
    public void run() {
        ...
        solutionsRanking = new HashMap<>();
        solutionsRegion = new HashMap<>();
        solutionsDistance = new HashMap<>();

        int numberOfSubfronts = ranking.getNumberOfSubfronts();
        for (int i = 0; i < numberOfSubfronts; i++) {
            List<DoubleSolution> solutions = ranking.getSubfront(i);
            for (Solution s : solutions) {
                solutionsRanking.put(s, i);
            }
        }

        int curRank;
        for (int i = 0; i < populationSize; i++) {
            curRank = solutionsRanking.get(population.get(i));
            rankIdx[curRank][i] = 1;
        }
        ...
        do {
            ...
            for (int i = 0; i < populationSize; i++) {
                ...
                List<DoubleSolution> parents = matingSelection(cid, type);
                ...
                // update ideal points
                updateIdealPoint(offSpring.get(0));
                updateIdealPoint(offSpring.get(1));

                // update nadir points
                updateNadirPoint(offSpring.get(0));
                updateNadirPoint(offSpring.get(1));
    ...
    public void setLocation(Solution indiv, double[] z_, double[] nz_) {
        ...
        solutionsRegion.put(indiv, minIdx);
        solutionsDistance.put(indiv, minDist);
    }
    ...
    public int nondominated_sorting_add(Solution indiv) {
        ...
        for (int i = 0; i < num_ranks; i++) {
            ...
            if (flag == 1) {	// 'indiv' is non-dominated with all solutions in the ith non-domination level, then 'indiv' belongs to the ith level
                solutionsRanking.put(indiv, (i - 1));
                return num_ranks;
                ...
                for (int j = 0; j < curListSize; j++) {
                    ...
                    solutionsRanking.put(population.get(curIdx), newRank);
                }
                    ...
                    if (allFlag == 1) {	// move the solutions after the 'prevRank'th level to their next levels
                        ...
                        while (curRank < num_ranks) {
                            ...
                            for (int j = 0; j < level_size; j++) {
                                ...
                                tempRank = solutionsRanking.get(population.get(curIdx));
                                newRank = tempRank + 1;
                                solutionsRanking.put(population.get(curIdx), newRank);
                ...
                while (i < num_ranks) {
                    ...
                    for (int j = 0; j < level_size; j++) {
                        curIdx = tempRecord[k][j];
                        curRank = solutionsRanking.get(population.get(curIdx));
                        newRank = curRank + 1;
                        solutionsRanking.put(population.get(curIdx), newRank);
        ...
        if (flag == 1) {
            solutionsRanking.put(indiv, level);
        } else if (flag == 2) {
            solutionsRanking.put(indiv, level);
            ...
            for (int i = 0; i < tempSize; i++) {
                ...
                solutionsRanking.put(population.get(curIdx), level + 1);
                rankIdx[level][curIdx] = 0;
                rankIdx[level + 1][curIdx] = 1;
            }
            num_ranks++;
        } else if (flag == 3) {
            solutionsRanking.put(indiv, level + 1);
            num_ranks++;
        } else {
            solutionsRanking.put(indiv, level);
            for (int i = 0; i < populationSize; i++) {
                if (rankIdx[level][i] == 1) {
                    solutionsRanking.put(population.get(i), level + 1);
                    rankIdx[level][i] = 0;
                    rankIdx[level + 1][i] = 1;
                }
            }
    ...
    public void replace(int position, DoubleSolution solution) {
        if (position > this.population.size()) {
            ...
            DoubleSolution removed = population.get(position);
            solutionsDistance.remove(removed);
            solutionsRanking.remove(removed);
            solutionsRegion.remove(removed);
    ...
    public void nondominated_sorting_delete(Solution indiv) {
        ...
        int indivRank = solutionsRanking.get(indiv);
                ...
                if (rankIdx[investigateRank][i] == 1) {
                    ...
                    if (checkDominance(indiv, population.get(i)) == 1) {
                        ...
                        if (flag == 0) {	// the ith solution can move to the prior level
                            ...
                            solutionsRanking.put(population.get(i), investigateRank - 1);
                        }
                    }
                }
            }
                        ...
                        if (rankIdx[investigateRank][j] == 1) {
                            ...
                            if (checkDominance(population.get(curIdx), population.get(j)) == 1) {
                                ...
                                if (flag == 0) {
                                    ...
                                    solutionsRanking.put(population.get(j), (investigateRank - 1));
                                }
                            }
                        }
                    }
    ...
    public void deleteCrowdRegion1(DoubleSolution indiv, int location) {
        ...
        int maxRank = solutionsRanking.get(population.get(indList.get(0)));
        maxRankList.add(indList.get(0));
        for (int i = 1; i < indList.size(); i++) {
            int curRank = solutionsRanking.get(population.get(indList.get(i)));
            if (curRank > maxRank) {
                maxRankList.clear();
                maxRank = curRank;
        ...
        int indivRank = solutionsRanking.get(indiv);
        int targetRank = solutionsRanking.get(population.get(targetIdx));
        rankIdx[targetRank][targetIdx] = 0;
        rankIdx[indivRank][targetIdx] = 1;

        Solution targetSol = population.get(targetIdx).copy();
    ...
    public void deleteCrowdRegion2(DoubleSolution indiv, int location) {
        ...
        for (int i = 1; i < indList.size(); i++) {
            int curRank;
            if (indList.get(i) == -1) {
                curRank = solutionsRanking.get(indiv);
            } else {
                curRank = solutionsRanking.get(population.get(indList.get(i)));
            }
    ...
    public void updateArchive(DoubleSolution indiv) {
        ...
        int location = solutionsRegion.get(indiv);
        ...
        if (numRanks == 1) {
            ...
            List<DoubleSolution> lastFront = new ArrayList<>(populationSize);
            ...
            if (frontSize == 0) {	// the last non-domination level only contains 'indiv'
                ...
                if (solutionsRanking.get(indiv) == (numRanks - 1)) {
                    frontSize++;
                    lastFront.add(indiv);
                }
            }
                ...
                if (nicheCount == 0) {
                    ...
                    if (list.isEmpty()) {
                        ...
                        if (idxArray[targetIdx] == -1) {
                            ...
                            int indivRank = solutionsRanking.get(indiv);
                            int targetRank = solutionsRanking.get(population.get(idxArray[targetIdx]));
                            rankIdx[targetRank][idxArray[targetIdx]] = 0;
                            rankIdx[indivRank][idxArray[targetIdx]] = 1;

                            Solution targetSol = population.get(idxArray[targetIdx]).copy();
    ...
    public List<DoubleSolution> matingSelection(int cid, int type) {
