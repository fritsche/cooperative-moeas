...
public class NSGAIII<S extends Solution<?>> extends AbstractGeneticAlgorithm<S, List<S>> {
    ...
    public NSGAIII(NSGAIIIBuilder<S> builder) { // can be created from the NSGAIIIBuilder within the same package
        ...
        String fileName = builder.getUniformWeightFileName();

        // if there is a Uniform Weight File: load it
        if (fileName != null && !fileName.isEmpty()) {
            (new ReferencePoint<S>()).loadFromFile(fileName, referencePoints, getProblem().getNumberOfObjectives());
        } else { // else: create the Weights
        ...
        JMetalLogger.logger.log(Level.INFO, "rpssize: {0}", referencePoints.size());
        JMetalLogger.logger.log(Level.INFO, "popsize: {0}", populationSize);
    }
